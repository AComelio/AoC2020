# -*- coding: utf-8 -*-
"""
Created on Sat Dec 19 16:57:36 2020
...
def part1(messages):
    z = getre_p1('0')
    ct = 0
    for string in messages.split('\n'):
        ct += bool(re.fullmatch(z, string))
    return ct

print(part1(messages))
...
def getre_p2(rulenum):
    ...
    if re.fullmatch(r'"."', rule):
        ...
        for part in parts:
            nums = part.split(' ')
            res.append(''.join(getre_p2(num) for num in nums))
        return '(?:' + '|'.join(res) + ')'
