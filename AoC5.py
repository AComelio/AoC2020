# -*- coding: utf-8 -*-
"""
Created on Sat Dec  5 10:52:33 2020
...
def part1(boarding_passes):
    return max([code_to_seat(boarding_pass)
                for boarding_pass in boarding_passes])

print('Highest seat id is %s' % part1(boarding_passes))
...
def part2(boarding_passes):
    taken_seats = sorted([code_to_seat(boarding_pass)
                          for boarding_pass in boarding_passes])
    for seat in taken_seats:
        if seat+1 not in taken_seats:
            return seat+1

print('My seat is %s' % part2(boarding_passes))
